...
export const createResendInviteEmailSender = ({
  ...
  return async ({ member, actor, invitePayload }) => {
    ...
    const ctaUrl = normalizedUrl;
    const html = `
      <div style="font-family: Arial, sans-serif; line-height: 1.5; color: #111">
        <p>${safeActorName} invited you to collaborate on <strong>${safeAppName}</strong>.</p>
        <p>Role: <strong>${safeRole}</strong></p>
        <p style="margin: 24px 0;">
          <a href="${ctaUrl}" style="background-color: #2563eb; color: #fff; padding: 12px 20px; border-radius: 6px; text-decoration: none; font-weight: bold;">
            Open ${safeAppName}
          </a>
        </p>
        <p>If the button doesn't work, copy and paste this link into your browser:</p>
        <p><a href="${ctaUrl}" style="color: #2563eb;">${ctaUrl}</a></p>
