...
const renderHtmlResponse = (options: { title: string; message: string; status?: number }) => {
...
serve(async (req) => {
  ...
  if ((recipient as RecipientRow).status !== "pending") {
    return renderHtmlResponse({
  ...
  if (method === "GET") {
    const recipientLabel = (recipient as RecipientRow).name || (recipient as RecipientRow).email || "Recipient";
  ...
  const { error: updateError } = await supabase
    ...
    .eq("id", (recipient as RecipientRow).id);
  ...
  const { data: siblingRecipients, error: siblingsError } = await supabase
    .from("change_order_recipients")
    .select("status")
    .eq("change_order_id", (recipient as RecipientRow).change_order_id);
  ...
  const { error: changeOrderError } = await supabase
    ...
    .eq("id", (recipient as RecipientRow).change_order_id);
